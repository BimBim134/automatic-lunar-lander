class plot {
    constructor(name_, x_, y_, vmin_, vmax_) {
        this.data = [];
        this.target = [];
        this.name = name_;
        ...
        this.draw = function(d_, t_) {
            ...
            } else if (t_ < this.vmin | t_ > this.vmax) {
                this.clipping = true;
                t = constrain(t_,this.vmin,this.vmax); // clipping the data
            } else {
                this.clipping = false;
            }
            ...
            stroke(0, 168, 255);
            for (let i = 0; i < 99; i++) {
                line(100 - i - 50, map(this.target[i], this.vmin, this.vmax, 50, -50), 100 - i - 51, map(this.target[i + 1], this.vmin, this.vmax, 50, -50));
            }
            fill(0, 168, 255);
            noStroke();
            textAlign(LEFT, CENTER);
            text(this.crop(this.target[0]), 55, map(this.target[0], this.vmin, this.vmax, 50, -50));
